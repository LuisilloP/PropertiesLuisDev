...
const PropertyCard = (props: Props) => {
    const { cant, next, newSearch, firstCall } = props
    ...
    if (firstCall == true) {
        propertyesCard = properties
    }
    return (

        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5'>
            {firstCall == false && propertyesCard.length === 0 ? <>no hay nada lo sentimos D: </> : propertyesCard.length === 0 ? <div role="status">
            ...
            </div> : propertyesCard.slice(0 || next, cant).map((property) => (
